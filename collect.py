...
def already_processed(source_url: str, university_name: str, content: str) -> bool:
    h = make_hash(source_url, university_name, content)
    try:
        res = supabase.table("festivals").select("id").eq("scraped_hash", h).execute()
